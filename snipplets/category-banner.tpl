...
<div class="banner-container" data-store="category-banner">
    ...
    {% endif %}
        <img class="banner-image" fetchpriority="high" src="{{ category_images['large'] }}" srcset="{{ category_images['large'] }} 480w, {{ category_images['huge'] }} 640w, {{ category_images['original'] }} 1024w, {{ category_images['1080p'] }} 1920w" alt="{{ 'Banner de la categoría' | translate }} {{ category.name }}"/>
    {% if not has_category_images and settings.banner_products_url != '' %}
        </a>
    {% endif %}
</div>